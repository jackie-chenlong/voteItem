...
signUp.uploadImage = function() {
	...
	// 最大上传图片数量  
	var maxCount = 2;
	$('#appendBtn').on('change', function(event) {
		//		event.cancelBubble = true;
		...
		for(var i = 0, len = files.length; i < len; i++) {
			...
			if($('#' + signUp.imageId).css('background-image') != 'none') {
				$('#loadingToast').hide();
				$.weui.alert({
					text: '图片已存在,请先删除'
				});
			...
			reader.onload = function(e) {
				...
				img.onload = function() {
					// 不要超出最大高度  
					var h = Math.min(img.height, maxHeight);
					// 宽度按比例计算  
					...
					// 插入到预览区
					$('#' + signUp.imageId).css('background-color', '');
					$('#' + signUp.imageId).children().hide();
					$('#' + signUp.imageId).css('background-image', 'url(' + base64 + ')');
					//					$('#' + signUp.imageId).addClass('weui_uploader_status').append('<div class="weui_uploader_status_content">0%</div>');
					$('#' + signUp.imageId).siblings().show();
					signUp.saveImage();
					// 然后假装在上传，可以post base64格式，也可以构造blob对象上传，也可以用微信JSSDK上传  			
					//					var progress = 0;
					//					function uploading() {
					//						$('#' + signUp.imageId).find('.weui_uploader_status_content').text(++progress + '%');
					//						if(progress < 100) {
